...
| `millennium_config` | Inspects Millennium client settings, manages plugin/theme activation, and isolates current-session, directly attributed component errors. | `action` (string, required): `show`, `plugins`, `themes`, `errors`, `enable`, `disable`, `theme`, `disable-theme`, or `disable-errors`. <br> `names` (string array, optional): plugin/theme names required by the selected action. <br> `dry_run` (boolean, optional): preview writes; invalid for read actions. <br> `confirm` (boolean, optional): required and valid only when applying `disable-errors`. <br> `plugins_only` / `themes_only` (boolean, optional): scope `disable-errors`; invalid for other actions and mutually exclusive. |
