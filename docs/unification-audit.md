...
| `schedule enable/disable/status` | Y | Y | Native (Phase 4) | `test_schedule` | `millennium-schedule` | systemd vs Task Scheduler |
| `schedule setup` wizard | Y | Y | Native (Phase 4) | Y | Y | Interactive both OSes |
| `schedule config get/set/list` | Y | Y | **Native Go (Phase 2)** | Y + Go | Y + Go | Config path graduated; timer actions still legacy |
