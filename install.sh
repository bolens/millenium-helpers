...
install_completions() {
  ...
  for comp in "${obsolete_completions[@]}"; do
    local f="${bash_dir}/${comp}"
    if [[ -f "$f" && -w "$bash_dir" ]]; then
      execute rm -f "$f"
    fi
  done
  for base_dir in "/usr/share" "/usr/local/share"; do
    local zsh_dir="${base_dir}/zsh/site-functions"
    for comp in "${obsolete_completions[@]}"; do
      local f="${zsh_dir}/_${comp}"
      if [[ -f "$f" && -w "$zsh_dir" ]]; then
        execute rm -f "$f"
      fi
    done
    local fish_dir="${base_dir}/fish/vendor_completions.d"
    for comp in "${obsolete_completions[@]}"; do
      local f="${fish_dir}/${comp}.fish"
      if [[ -f "$f" && -w "$fish_dir" ]]; then
        execute rm -f "$f"
      fi
    done
  done
