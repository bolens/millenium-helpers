...
function Show-DiagInstallMethod {
    ...
    switch ($script:InstallMethod) {
        'scoop'  { Print-DiagItem -Status 'ok'    -Label 'Install method' -Value 'Scoop package (millennium-helpers)' }
        'winget' { Print-DiagItem -Status 'ok'    -Label 'Install method' -Value 'Winget package (bolens.millenniumhelpers)' }
        'manual' { Print-DiagItem -Status 'ok'    -Label 'Install method' -Value 'Manual install (install.ps1 / release zip)' }
